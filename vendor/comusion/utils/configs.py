...
class Config:
    ...
    def __init__(self, cfg_id, test=False):
        ...
        cfg_dir_name = f'{cfg_id}_test' if test else cfg_id
        self.cfg_dir = '%s/%s' % (self.base_dir, cfg_dir_name)
        ...
        self.dataset = self.data_specs.get('dataset', 'assembly')
        ...
        self.humanmac_multimodal_threshold = self.data_specs.get('humanmac_multimodal_threshold', self.multimodal_threshold)
        self.data_aug = self.data_specs.get('data_aug', True)
        self.rota_prob = self.data_specs.get('rota_prob', 1.0)
        self.data_dir = self.data_specs.get('data_dir', '')
        self.action_filter = self.data_specs.get('action_filter', 'pick_up_screwd')
        ...
        self.early_stopping_enabled = self.learn_specs.get('early_stopping_enabled', False)
        self.early_stopping_patience = self.learn_specs.get('early_stopping_patience', 20)
        self.early_stopping_min_delta = self.learn_specs.get('early_stopping_min_delta', 1e-4)
        self.early_stopping_warmup = self.learn_specs.get('early_stopping_warmup', 0)
        self.early_stopping_monitor = self.learn_specs.get('early_stopping_monitor', 'train_loss')
        ...
        self.final_checkpoint_path = self.logging_specs.get('final_checkpoint_path', '')
        self.remove_model_internals = cfg.get('remove_model_internals', False)
