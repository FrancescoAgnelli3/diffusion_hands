...
from util.compat import ensure_torch_six_compat
ensure_torch_six_compat()
import timm
try:
    from timm.optim import param_groups_weight_decay as _param_groups_weight_decay
except Exception:
    _param_groups_weight_decay = None
...
def _build_param_groups(model, weight_decay):
    # timm>=1.0 API
    if _param_groups_weight_decay is not None:
        return _param_groups_weight_decay(model, weight_decay=weight_decay)
    # timm<1.0 provided add_weight_decay; some newer releases expose param_groups_weight_decay.
    ...
    raise AttributeError("No compatible weight decay grouping helper found in timm.optim.optim_factory")
